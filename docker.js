...
class DockerLambda {
  constructor (_path, entry, env, handler, runtime) {
    ...
    const envArray = Object.keys(env)
      .map(k => ['--env', k + '=' + env[k]])
        ...
        fs.writeFile(
          ...
          (err) => {
            ...
            this.proc = log(cp.spawn('docker', ['build', tmp, '-t', id]))
              .on('exit', (code) => {
                if (code) { return reject(new Error('docker build failed')) }

                // XXX: I think this should error but that breaks the tests...
                if (this.closed) { return resolve() }
                // reject(new Error('closed during startup'))

                console.log(['>docker', 'run', '-p', `${this.port}:8080`].concat(envArray).concat([id]).join(' '))
