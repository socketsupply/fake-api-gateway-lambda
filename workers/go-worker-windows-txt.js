...
func Run(input Input) ([]byte, error) {
  ...
  client, err := rpc.Dial("tcp", fmt.Sprintf("localhost:%d", input.Port))
  if err != nil {
    return nil, err
  }
...
func (input *Input) startLambdaIfNotRunning() func() {
  conn, err := net.DialTimeout("tcp", net.JoinHostPort("localhost", strconv.Itoa(input.Port)), input.TimeOut)
  if err != nil {
